<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container-fluid">
        <img src="../assets/img/logo/logo.png" width="50px" alt="">
        <div class="collapse navbar-collapse  justify-content-center" id="navbarNav">
            <h2 class="text-light">HEADER</h2>
        </div>
    </div>
</nav>

<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    <div class="container-fluid">
        <div class="collapse navbar-collapse justify-content-start" id=" navbarNav">
            <ul class="nav ">
                <li class="nav-item">
                    <a class="nav-link text-light text-uppercase " aria-current="page" href="#">HOME</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-light text-uppercase " aria-current="page" href="mod_print/laporan_temuan.php">Data Temuan</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-light text-uppercase " aria-current="page" href="mod_print/laporan_realisasi.php">Data realisasi</a>
                </li>
            </ul>
        </div>
        <div class="collapse navbar-collapse justify-content-end" id=" navbarNav">
            <ul class="nav ">
                <li class="nav-item">
                    <a class="nav-link text-light text-uppercase " aria-current="page" href="../logout.php">logout</a>
                </li>
            </ul>
        </div>
    </div>
</nav>